...
titles_data = [line.strip().lower() for line in open('../titlelist.txt', 'r').readlines()]
missing = [58,296,661,1133,1149,1415]
...
for i in range (0,len(data)):
	...
	index = titles_data.index(title.strip().lower()) # index of the paper in the corpus
	if index in missing:
		p = paper(title, doi, url, year, abstract, [(i,0) for i in range(8) ])
		paper_corpus[title] = p
		continue
	offset = sum(m<index for m in missing)

	topic_dist = topic_dist_data[str(index-offset)]
